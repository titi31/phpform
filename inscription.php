...
<br>
			...
			switch ($_REQUEST ['choix']) {
				case 'inscription':
					...
					case 'connecter':
							...
							echo "<div id='monCompte'>".$membre->getPseudo()."<br> ".$membre->getMail()."<br> ".$membre->getChaineMots()."
								<form method='post'><input type='hidden' value='".$_REQUEST['id']."' name='id'>
								<button type='submit' value='deluser' name='choix'>delete user</button><br>
								
								<button type='submit' value='upuser' name='choix'>mettre a jour user</button></div>
	...
	/*function enregistrerForm(){
		...
		$sql = "INSERT INTO Membres(pseudo ,mail, chaineMots) VALUES(?, ?, ?);";
						...
						$stmt->bindParam(1, $_REQUEST['pseudo']);
						$stmt->bindParam(2, $_REQUEST['mail']);
						$stmt->bindParam(3, $_REQUEST['chaineMots']);
						if ($stmt->execute()==true)
							$id = $bdd->lastInsertId(); // recup id auto-incrementé généré par la base
						else 
...
function modifier($membre){
	...
	function afficherAccueil(){
		...
		<div id='boutonInscrire'><button type='submit'value='inscription' name='choix'>inscription</button></div><br>
